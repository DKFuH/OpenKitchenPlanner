export const de = {
  common: {
    back: '← Zurück',
    save: 'Speichern',
    saving: 'Wird gespeichert…',
    cancel: 'Abbrechen',
    loading: 'Lade…',
    error: 'Fehler',
    success: 'Gespeichert.',
    language: 'Sprache',
  },
  nav: {
    settings: 'Einstellungen',
    projects: 'Projekte',
    catalog: 'Katalog',
    dashboard: 'Dashboard',
  },
  plugins: {
    fengShui: 'Feng Shui',
  },
  shell: {
    brandName: 'OKP',
    brandTagline: 'Einheitliche Planung und Ausfuehrung',
    navigationAriaLabel: 'Hauptnavigation',
    navigation: {
      projects: 'Projekte',
      catalog: 'Katalog',
      documents: 'Dokumente',
      contacts: 'Kontakte',
      reports: 'Reports',
      settings: 'Einstellungen',
      editor: 'Editor',
      presentation: 'Praesentation',
      exports: 'Exporte',
      panoramaTours: 'Panorama-Touren',
      quoteLines: 'Angebotspositionen',
      specificationPackages: 'Leistungspakete',
    },
    backend: {
      menu: 'Backend-Features',
      quoteLines: 'Angebotspositionen',
      panoramaTours: 'Panorama-Touren',
      specificationPackages: 'Leistungspakete',
      viewerExports: 'Viewer-Exporte',
      presentationMode: 'Praesentation',
      cameraPanel: 'Kamera-Panel',
      capturePanel: 'Capture-Panel',
      renderEnvironmentPanel: 'Render-Umgebung',
      daylightPanel: 'Tageslicht-Panel',
      materialPanel: 'Material-Panel',
      // Kanban-Befehle
      newProject: 'Neues Projekt',
      openInEditor: 'Im Editor öffnen',
      archiveProject: 'Archivieren',
      deleteProject: 'Löschen',
      statusLead: 'Lead',
      statusPlanning: 'Planung',
      statusQuoted: 'Angebot',
      statusContract: 'Auftrag',
      statusProduction: 'Produktion',
      statusInstalled: 'Montage',
      duplicateProject: 'Duplizieren',
      customerData: 'Kundendaten',
      companySettings: 'Firmeneinstellungen',
      help: 'Hilfe',
      about: 'Über OKP',
    },
    plugins: {
      menu: 'Plugins',
    },
    mcp: {
      menu: 'MCP',
      openHub: 'MCP-Hub oeffnen',
      openProjectContext: 'Projektkontext im MCP-Hub',
      copyValidationPrompt: 'Validierungs-Prompt kopieren',
      copied: 'Kopiert',
    },
    workflow: {
      steps: {
        walls: 'Waende',
        openings: 'Oeffnungen',
        furniture: 'Moebel',
      },
    },
    mode: {
      label: 'Modus',
    },
    badges: {
      projectBound: 'Projektgebunden',
      globalContext: 'Global',
    },
    actions: {
      previousStep: 'Zurueck',
      nextStep: 'Weiter',
      previousStepDisabled: 'Bereits im ersten Workflow-Schritt',
      nextStepDisabled: 'Bereits im letzten Workflow-Schritt',
      disabledFallback: 'Diese Aktion ist aktuell nicht verfuegbar',
    },
    reasons: {
      projectContextMissing: 'Projektkontext fehlt',
      pluginDisabledForTenant: 'Plugin ist fuer diesen Tenant deaktiviert',
      noDirectUiEntry: 'Kein direkter UI-Einstiegspunkt verfuegbar',
    },
  },
  settings: {
    title: 'Einstellungen',
    subtitle: 'Verwalte Firmenprofil und aktivierte Plugins.',
    sections: 'Bereiche',
    companyProfile: 'Firmenprofil',
    plugins: 'Plugins',
    layoutStyles: 'Layout-Stile',
    languagePacks: 'Sprachpakete',
    languagePacksSubtitle: 'Verwalte tenant-spezifische Terminologie und Übersetzungs-Overrides.',
    languageSection: 'Sprache / Language',
    languageHint: 'Wähle die Anzeigesprache der Anwendung.',
    companyTitle: 'Firmenprofil',
    companySubtitle: 'Firmendaten für Angebots-PDF, Steuer- und Bankverbindung',
    tenantLocale: 'Standard-Sprache (Mandant)',
    tenantLocaleSave: 'Mandanten-Sprache speichern',
    tenantLocaleSaved: 'Mandanten-Sprache gespeichert.',
    languagePackCreate: 'Sprachpaket anlegen',
    languagePackList: 'Sprachpakete',
    languagePackLocale: 'Locale-Code',
    languagePackName: 'Name',
    languagePackJson: 'Nachrichten (JSON)',
    languagePackSave: 'Sprachpaket speichern',
    languagePackDelete: 'Sprachpaket löschen',
    languagePackCreated: 'Sprachpaket erstellt.',
    languagePackSaved: 'Sprachpaket gespeichert.',
    languagePackDeleted: 'Sprachpaket gelöscht.',
    languagePackNoItems: 'Keine Sprachpakete vorhanden.',
    languagePackInvalidJson: 'Ungültiges JSON-Format für Nachrichten.',
    languagePackNameRequired: 'Bitte einen Namen für das Sprachpaket angeben.',
    languagePackSaveError: 'Sprachpaket konnte nicht gespeichert werden.',
    languagePackLoadError: 'Sprachpakete konnten nicht geladen werden.',
  },
  projects: {
    title: 'Projekte',
    newProject: 'Neues Projekt',
    noProjects: 'Keine Projekte gefunden.',
    loadError: 'Fehler beim Laden der Projekte',
    createError: 'Fehler beim Anlegen',
    duplicateError: 'Duplizieren fehlgeschlagen',
    archiveError: 'Archivieren fehlgeschlagen',
    deleteConfirm: 'Projekt wirklich löschen?',
    noDeadline: 'Kein Termin',
    priorityLow: 'Niedrig',
    priorityMedium: 'Mittel',
    priorityHigh: 'Hoch',
    statusLead: 'Lead',
    statusPlanning: 'Planung',
    statusQuoted: 'Angebot',
    statusContract: 'Auftrag',
    statusProduction: 'Produktion',
    statusInstalled: 'Montage',
  },
  ribbon: {
    quickAccess: {
      ariaLabel: 'Schnellzugriff',
    },
    overflow: {
      more: 'Mehr',
    },
    tabs: {
      datei: 'Datei',
      start: 'Start',
      einfuegen: 'Einfügen',
      cad: 'CAD',
      ansicht: 'Ansicht',
      render: 'Render',
      daten: 'Daten',
      plugins: 'Plugins',
      // Kanban-Bereich
      projekt: 'Projekt',
      aendern: 'Ändern',
      einstellungen: 'Einstellungen',
      hilfe: 'Hilfe',
    },
    contextTabs: {
      wandtools: 'Wandtools',
      oeffnung: 'Öffnung',
      objekt: 'Objekt',
      presentation: 'Präsentation',
    },
    groups: {
      file: 'Datei',
      export: 'Export',
      clipboard: 'Zwischenablage',
      selection: 'Auswahl',
      workflow: 'Workflow',
      elements: 'Elemente',
      annotation: 'Beschriftung',
      assets: 'Assets',
      architektur: 'Architektur',
      import: 'Import',
      draw: 'Zeichnen',
      edit: 'Bearbeiten',
      snap: 'Fang / Ausrichtung',
      view: 'Ansicht',
      visibility: 'Sichtbarkeit',
      locking: 'Sperren',
      cameraPresets: 'Kamera-Presets',
      screenshot: 'Screenshot',
      renderEnvironment: 'Umgebung',
      presentation: 'Präsentation',
      workflows: 'Workflows',
      interop: 'Interop',
      erpHooks: 'ERP / Hooks',
      tenantPlugins: 'Tenant-Plugins',
      mcpActions: 'MCP-Aktionen',
      material: 'Material',
      // Kanban-Gruppen
      newProject: 'Neu',
      projectActions: 'Projekt',
      manage: 'Verwalten',
      status: 'Status',
      projectChange: 'Projekt',
      system: 'System',
      info: 'Info',
    },
    commands: {
      new: 'Neu',
      open: 'Öffnen',
      save: 'Speichern',
      duplicate: 'Duplizieren',
      gltfExport: 'GLB-Export',
      viewerExports: 'Viewer-Exporte',
      undo: 'Rückgängig',
      redo: 'Wiederholen',
      cut: 'Ausschneiden',
      copy: 'Kopieren',
      paste: 'Einfügen',
      selectAll: 'Alles auswählen',
      deselect: 'Auswahl aufheben',
      previousStep: 'Zurück',
      nextStep: 'Weiter',
      autoComplete: 'Auto-Vervollst.',
      insertWindow: 'Fenster',
      insertDoor: 'Tür',
      insertFurniture: 'Möbel',
      insertStairs: 'Treppen',
      insertRoofSlope: 'Dachschräge',
      importFile: 'Import',
      importDxf: 'DXF/DWG',
      importIfc: 'IFC',
      importSketchup: 'SketchUp',
      insertLabel: 'Label',
      insertDim: 'Bemaßung',
      assetLibrary: 'Asset-Bibliothek',
      wall: 'Wand',
      room: 'Raum',
      polyline: 'Polylinie',
      select: 'Auswahl',
      pan: 'Pan',
      calibrate: 'Kalibrieren',
      pointSnap: 'Punktfang',
      axisSnap: 'Achsenfang',
      angleSnap: 'Winkelfang',
      safeEdit: 'Safe Edit',
      areas: 'Bereiche',
      snapAlign: 'Fang / Ausrichtung',
      topology: 'Topologie',
      view2d: '2D',
      viewSplit: 'Split',
      viewSplit3: '3-Fach',
      view3d: '3D',
      viewElevation: 'Elevation',
      viewSection: 'Schnitt',
      resetLayout: 'Standardlayout',
      activeLevelVisibility: 'Ebene sichtbar',
      dimensionsVisibility: 'Bemaßung sichtbar',
      placementsVisibility: 'Objekte sichtbar',
      activeLevelLock: 'Ebene sperren',
      dimensionsLock: 'Bemaßung sperren',
      visibility: 'Sichtbarkeit',
      grid: 'Raster',
      cameraTop: 'Draufsicht',
      cameraPerspective: 'Perspektive',
      cameraPanel: 'Kamera-Panel',
      screenshot: 'Screenshot',
      capture360: '360-Export',
      capturePanel: 'Capture-Panel',
      renderEnvironmentPanel: 'Render-Umgebung',
      daylightPanel: 'Tageslicht-Panel',
      presentationMode: 'Präsentation',
      panoramaTours: 'Panorama-Touren',
      reports: 'Reports',
      quoteLines: 'Angebotspositionen',
      specificationPackages: 'Leistungspakete',
      documents: 'Dokumente',
      contacts: 'Kontakte',
      pluginSettings: 'Plugin-Einstellungen',
      materialPanel: 'Material-Panel',
      // Kanban-Befehle
      newProject: 'Neues Projekt',
      openInEditor: 'Im Editor öffnen',
      archiveProject: 'Archivieren',
      deleteProject: 'Löschen',
      statusLead: 'Lead',
      statusPlanning: 'Planung',
      statusQuoted: 'Angebot',
      statusContract: 'Auftrag',
      statusProduction: 'Produktion',
      statusInstalled: 'Montage',
      duplicateProject: 'Duplizieren',
      customerData: 'Kundendaten',
      companySettings: 'Firmeneinstellungen',
      help: 'Hilfe',
      about: 'Über OKP',
    },
    reasons: {
      alreadyFirstStep: 'Bereits im ersten Workflow-Schritt',
      alreadyLastStep: 'Bereits im letzten Workflow-Schritt',
      noProjectSelected: 'Bitte zuerst ein Projekt auswählen',
    },
  },
  editor: {
    lockState: {
      unknownUser: 'Unbekannt',
    },
  },
}

export type Messages = typeof de
