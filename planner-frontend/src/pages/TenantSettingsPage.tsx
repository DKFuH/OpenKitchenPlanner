...
export function TenantSettingsPage() {
  ...
  useEffect(() => {
    void fetch('/api/v1/tenant/locale-settings')
      .then((r) => (
        r.ok
          ? r.json() as Promise<{ preferred_locale: string | null; fallback_locale: string | null }>
          : Promise.reject(r)
      ))
      .then((data) => {
        if (data.preferred_locale) {
          setTenantLocale(data.preferred_locale)
        }
      })
      .catch(() => undefined)
  }, [])
  ...
  async function handleSaveTenantLocale() {
    ...
    try {
      const response = await fetch('/api/v1/tenant/locale-settings', {
        method: 'PUT',
      ...
      if (response.ok) {
        setLocaleSaveMsg(t('settings.tenantLocaleSaved'))
      }
    } catch {
      // not critical for page usability
    }
  }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const { id: _id, tenant_id: _tenantId, created_at: _createdAt, updated_at: _updatedAt, ...payload } = form
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <Body1 style={{ color: tokens.colorNeutralForeground3, display: 'block' }}>
            Firmendaten fuer Angebots-PDF, Steuer- und Bankverbindung
          </Body1>
        </div>
      ...
      <form onSubmit={(event) => void handleSubmit(event)}>
        <section className={styles.section}>
          ...
          <div className={styles.grid}>
            ...
            <Field label="Strasse"><Input maxLength={200} value={form.company_street ?? ''} onChange={(e) => handleChange('company_street', e.target.value)} /></Field>
        ...
        <section className={styles.section} style={{ marginTop: tokens.spacingVerticalL }}>
          <Subtitle2>Steuer und Bank</Subtitle2>
          <div className={styles.grid}>
        ...
        <section className={styles.section} style={{ marginTop: tokens.spacingVerticalL }}>
          ...
          <div className={styles.grid}>
            <Field label="Fusstext (erscheint auf jedem Angebots-PDF)" className={styles.fullWidth}>
              <Textarea rows={4} maxLength={2000} value={form.quote_footer ?? ''} onChange={(_e, data) => handleChange('quote_footer', data.value)} />
            </Field>
            <Field label="Waehrung"><Input maxLength={3} placeholder="EUR" value={form.currency_code ?? ''} onChange={(e) => handleChange('currency_code', e.target.value)} /></Field>
          </div>
        </section>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.actions}>
          <div>
            ...
            <Select value={tenantLocale} onChange={(_e, data) => { setTenantLocale(data.value); setLocaleSaveMsg(null) }}>
              <Option value="">-</Option>
